...
interface UseGanttStateOptions<TData = unknown> extends GanttCallbacks<TData> {
  ...
  /**
   * Explicit finite axis range. Unlike rangeBounds (which only constrains
   * navigation), this is the coordinate range used by the headers, bars and
   * pointer gestures.
   */
...
function createGanttStore<TData>(
  ...
  const getState = (): GanttState<TData> => {
    ...
    const { visibleRange: defaultVisibleRange, activeRange } = getGanttDateRange(
      scale,
      date,
      rangeOpts
    )
    const baseRange = options.visibleRange ?? defaultVisibleRange
