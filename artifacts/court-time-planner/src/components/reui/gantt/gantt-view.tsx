  ...
  const { units, groups, unitWidthRem } = useMemo(() => {
    ...
    if (scale === "day") {
      ...
      while (dayCursor.getTime() < rangeEndMs) {
        ...
        // A finite day range may begin/end inside the calendar day. Build only
        // the clipped units, rather than the whole 00:00–24:00 day. The unit
        // weights are the same coordinate system used by bars and gestures.
        const dayStartMin = Math.max(
          0,
          (range.start.getTime() - dayCursor.getTime()) / 60000
        )
        const dayEndMin = Math.min(
          dayMinutes,
          (range.end.getTime() - dayCursor.getTime()) / 60000
        )
        let span = 0
        for (let m = dayStartMin; m < dayEndMin; m += interval) {
          ...
          const weight = Math.min(interval, dayEndMin - m) / interval
          units.push({
