...
export default function RosterPage() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      ...
      { key: "no-warning", label: "No process warning", count: all.filter(c => !c.waitingOn).length },
      { key: "confirm", label: "Process to confirm", count: all.filter(c => Boolean(c.waitingOn)).length },
    ...
    <div className="workspace-page space-y-6 pb-8" data-testid="roster-page">
      <header className="workspace-header">
        ...
        <div>
          ...
          <p className="workspace-subtitle">Browse the working roster by stage. Process warnings are inferred and need confirmation.</p>
        </div>
      </header>
      ...
      <section className="flex flex-wrap items-center gap-3" aria-label="Roster controls">
            <Popover>
              ...
              <PopoverContent align="start" className="w-80 space-y-4">
                ...
                <fieldset><legend className="text-xs font-medium text-muted-foreground">Process warning (inferred)</legend><div className="mt-1 grid grid-cols-3 gap-1">{[["", "Any"], ["ready", "None found"], ["waiting", "To confirm"]].map(([value, label]) => <button type="button" key={value} data-testid={`button-readiness-${value || "any"}`} onClick={() => setReadinessFilter(value as Readiness)} className={`rounded-md border px-2 py-1.5 text-xs ${readinessFilter === value ? "border-primary bg-primary/10 text-primary" : "hover:bg-muted"}`}>{label}</button>)}</div></fieldset>
          ...
          <div id={`stage-cases-${index}`} role="region" aria-label={`${stage} cases`} hidden={!isOpen} className="border-t border-border">
            {stageCases.map((c) => <Link href={`/roster/${encodeURIComponent(c.id)}`} key={c.id} data-testid={`link-case-${c.id}`} title={`Latest hearing: ${narrativeExcerpt(c.lastHearing)}`} className="flex items-center justify-between gap-4 border-b border-border px-5 py-3.5 transition-colors hover:bg-muted/50 last:border-b-0">
              ...
              <div className="flex shrink-0 items-center gap-3 text-[12px]">
                ...
                {c.waitingOn && <span className="rounded-full border border-rose-100 bg-rose-50 px-2 py-0.5 text-[11px] font-medium text-rose-700">To confirm</span>}
                {!c.waitingOn && c.flags.length > 0 && <span className="rounded-full border border-blue-100 bg-blue-50 px-2 py-0.5 text-[11px] font-medium text-blue-700">{c.flags[0].replaceAll("_", " ").toLowerCase()}</span>}
